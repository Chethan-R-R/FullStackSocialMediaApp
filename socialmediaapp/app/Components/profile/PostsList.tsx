        ...
        async function allPost() {
            ...
            if(data){
                const list=data.map((post:Post)=>{
                    return(
                        <Link href={`post/${user._id}/${post._id}`} key={post._id} className="singlepost" onClick={()=>context.handleNav('.main','.profile','.more')}>
                            ...
                            <img src={`${process.env.NEXT_PUBLIC_API}/uploads/${post.post_url}`} />
