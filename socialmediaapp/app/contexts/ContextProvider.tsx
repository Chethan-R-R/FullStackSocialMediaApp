...
const ProvideContext=({children}:{children:React.ReactNode})=>{
    ...
    const [infolist2,setInfolist2]=useState<string[]>("All the users data gets autometically deleted every 24 hours".split(" "))
    ...
    useEffect(()=>{
        const SocketData=io(`${process.env.NEXT_PUBLIC_API}`,{
            auth:{token:token}
        })
    ...
    async function handlePostUpload(){
        ...
        if(uploadpostdetails.post_picture!==""){
            if(token){
            displayLoading(true)
